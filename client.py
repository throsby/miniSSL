...
def validate_certificate(recv_certificate):
    if not keyutils.verify_certificate(readCertificate(path_to_ca_cert),
            recv_certificate):
        print 'Bad Certificate'
        return 0
    elif keyutils.read_issuer(recv_certificate) \
        != keyutils.read_issuer(readCertificate('certs/minissl-ca.pem'
                                )):
        print 'Bad Issuer'
        return 0
    else:
...
def initialise(socket):
    ...
    private_key = \
        keyutils.read_privkey_from_pem(readCertificate(CLIENT_PRIVATEKEY))
    ...
    session_key_one = keyutils.create_hmac(secret,
            initResponse[RECV_NONCE] + initNonce + '00000000')
    session_key_two = keyutils.create_hmac(secret,
            initResponse[RECV_NONCE] + initNonce + '11111111')

    msgHMAC = keyutils.create_hmac(session_key_two, all_recv_msgs)
    encrypted = keyutils.encrypt_with_rsa_hybrid(secret, public_key)

    confirm_msg = ('ClientKex:', encrypted[0], encrypted[1],
                   encrypted[2], msgHMAC)
